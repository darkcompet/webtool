...
public class UserModel : AutoGenerateUpdateTime {
	...
	/// It can be used for login instead of email.
	[Required]
	...
	/// When a client exceeds limit count of login via email/password, then we lock the login from the user for a while.
	[Column("login_locked_until")]
...
public class UserModelBuilder {
	...
	public static void OnModelCreating(ModelBuilder modelBuilder) {
		...
		modelBuilder.Entity<UserModel>().Property(model => model.number).ValueGeneratedOnAdd().UseIdentityColumn(999, 1);
